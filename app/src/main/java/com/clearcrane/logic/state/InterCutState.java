...
public class InterCutState extends PrisonBaseModeState {
	...
	public void startPlay() {
		...
		if (interCutType.equals("audio")) {
			...
			Message msg = mHandler.obtainMessage(ClearConstant.MSG_START_INTER_CUT);
			msg.obj = mPrisonOrganism.getWorkOrgan();
			msg.sendToTarget();	
		}
	...
	public void stopPlay() {
		...
		if (interCutType.equals("audio")) {
			...
			mHandler.obtainMessage(ClearConstant.MSG_STOP_INTER_CUT).sendToTarget();
	...
	public void init(Context context, Handler handler) {
		...
		mPreference = mContext.getSharedPreferences(ClearConstant.STR_INTER_CUT, Context.MODE_PRIVATE);
		initStateParams();
		initPrisonOrganism();
	}
	...
	public void initPrisonOrganism() {
		InterCutOrgan organ = new InterCutOrgan(sourcrUrl, interCutType, interCutTitle);
		if(interCutType.equals("audio")){
			organ = new InterCutOrgan(sourcrUrl, songName, duration);
		}
		organ.init(startTime, endTime);
		mPrisonOrganism = new InterCutObject(organ);
