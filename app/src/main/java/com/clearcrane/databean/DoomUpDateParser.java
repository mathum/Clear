...
public class DoomUpDateParser implements UpDateParser {
	...
	public List<UpDate> parse(InputStream is) throws Exception {
		...
		NodeList items = rootElement.getElementsByTagName("update");
		...
		if(nodeName.equals("model")){
			update.setModel(property.getFirstChild().getNodeValue());
		}else if(nodeName.equals("version")){
			update.setVersion(property.getFirstChild().getNodeValue());
		}else if(nodeName.equals("apkurl")){
			update.setApkurl(property.getFirstChild().getNodeValue());
		}else if(nodeName.equals("apkname")){
			update.setApkname(property.getFirstChild().getNodeValue());
		}else if(nodeName.equals("md5")){
			update.setMd5(property.getFirstChild().getNodeValue());
		}
		}
		updates.add(update);
		}
	...
	public String serialize(List<UpDate> updatas) throws Exception {
		...
		for (UpDate update : updatas){
			Element bookElement = doc.createElement("book");
//			bookElement.setAttribute("id", update.getId() + "");
